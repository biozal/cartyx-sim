...
- **The runaway-loop backstop pauses the session** rather than ending it: after 12 consecutive silent turns (in or out of combat) or 3 consecutive hand-offs no player character can answer, and a DM beat is capped at 24 tool calls before `hand_off` is forced. Every limit is a `DirectorConfig` option. (Plan 2A)
